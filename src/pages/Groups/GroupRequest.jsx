import GroupRequestCard from "../../components/GroupRequest/GroupRequestCard";
import { usePendingGroupInvites } from "../../hooks/group/usePendingGroupInvites";
import styles from "./GroupRequest.module.css";

export default function GroupRequests() {
  const {
    data: invites = [],
    isLoading,
    isError,
  } = usePendingGroupInvites();

  if (isLoading) {
    return (
      <div className={styles["group-requests-loading"]}>
        Loading Group Invitations...
      </div>
    );
  }

  if (isError) {
    return (
      <div className={styles["group-requests-loading"]}>
        Unable to load invitations
      </div>
    );
  }

  return (
    <div className={styles["group-requests-page"]}>
      <div className={styles["group-requests-header"]}>
        <h1>Group Invitations</h1>

        <p>
          Accept or reject invitations from your friends.
        </p>
      </div>

      {invites.length === 0 ? (
        <div className={styles["group-requests-empty"]}>
          <img
            src="https://cdn-icons-png.flaticon.com/512/4076/4076478.png"
            alt="No Invitations"
          />

          <h2>No Pending Invitations</h2>

          <p>
            You don't have any pending group invitations.
          </p>
        </div>
      ) : (
        <div className={styles["group-requests-list"]}>
          {invites.map((invite) => (
            <GroupRequestCard
              key={invite._id}
              invite={invite}
            />
          ))}
        </div>
      )}
    </div>
  );
}